...
$products_data = json_decode(base64_decode('""" + __import__('base64').b64encode(json.dumps(products).encode()).decode() + """'), true);
...
with open('generate_catalog.py', 'w') as f:
    f.write(f"""import ftplib
import os
import requests
...
try:
    ...
    # Upload PHP script
    with open('setup_catalog.php', 'w') as f:
        f.write({repr(php_script)})
    ...
    for file in os.listdir(img_dir):
        if file.endswith('.png'):
            with open(os.path.join(img_dir, file), 'rb') as f:
                ...
                ftp.storbinary(f'STOR staging/product_images/{{file}}', f)
                
    response = requests.get('https://bactiveph.com/staging/setup_catalog.php')
