...
Invalid new assignments are rejected. Native WooCommerce and variation AJAX saves retain their former image choices while saving unrelated fields, with WooCommerce errors explaining how to correct the image. REST rejects invalid incoming image IDs before invoking the Woo callback, including the complete create/update batch envelope; CSV imports validate all image choices before WooCommerce’s intermediate image-helper save and report a failed row. CSV image references must identify existing reviewed Media Library attachments by exact local URL, upload-relative path or attachment ID; ambiguous basenames and remote downloads are rejected. REST image URL imports must upload and review the image through Media Library first, then assign its attachment ID. Legacy unchanged images do not block price/stock maintenance. Publishing a new/draft/scheduled product checks all selected photos, including published variations. Scheduled publication rechecks the exact file reviews immediately before WordPress cron publishes; failures keep it as a draft. When correcting older invalid draft images in the classic editor, save corrections before publishing because WordPress updates publication status before WooCommerce saves its image metadata.
...
Replacing the bytes behind an already-live attachment invalidates its photo review immediately, but existing published assignments remain visible for continuity. Such replacements require the catalogue audit and fresh review; the policy blocks reuse and new publication, rather than automatically hiding live products. Plugins that call `wp_publish_post()` directly bypass WordPress’s pre-insert filter; scheduled publication is explicitly protected through its cron hook. Direct SQL remains outside these hooks.
