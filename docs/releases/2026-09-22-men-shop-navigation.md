...
## Bottoms activation — September 25, 2026 UTC

The first men's shorts product is now published in the actual child category `bottoms`, with canonical URL `/collections/men/bottoms/`. The earlier proposed `bottoms-men` slug was never the correct live destination. PR #144 corrected the lookup/link, and desktop/mobile live verification passed. Use the existing category; do not create or rename it to satisfy the old instruction. See [the release receipt](../work-records/2026-09-24-mens-bottoms.md) for backup, hashes, tests, cache verification and rollback evidence.
