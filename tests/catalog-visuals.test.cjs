...
test('native thumbnails gain keyboard activation, track replacements, and restore on fallback', async () => {
    const p = await page({productId:99});
    const gallery = p.w.document.createElement('div');
    gallery.className = 'woocommerce-product-gallery';
    gallery.innerHTML = '<div class="flexy-pills"><ol><li class="active"><span aria-label="Slide 1">First</span></li><li><span aria-label="Slide 2">Second</span></li></ol></div>';
    p.w.document.querySelector('.product').prepend(gallery);
    let clicked = 0;
    gallery.addEventListener('click', () => clicked++);
    p.w.bactiveCatalogVisuals = config;
    p.w.eval(source); p.form.trigger('wc_variation_form');
    const first = gallery.querySelector('span');
    assert.equal(first.getAttribute('role'), 'button');
    assert.equal(first.getAttribute('aria-pressed'), 'true');
    first.dispatchEvent(new p.w.KeyboardEvent('keydown', {key:'Enter', bubbles:true, cancelable:true}));
    assert.equal(clicked, 1);
    gallery.innerHTML = '<div class="flexy-pills"><ol><li><span aria-label="Slide 1">Replacement</span></li></ol></div>';
    await new Promise(resolve => p.w.setTimeout(resolve, 0));
    const replacement = gallery.querySelector('span');
    assert.equal(replacement.tabIndex, 0);
    assert.equal(replacement.getAttribute('aria-pressed'), 'false');
    replacement.parentElement.classList.add('active');
    await new Promise(resolve => p.w.setTimeout(resolve, 0));
    assert.equal(replacement.getAttribute('aria-pressed'), 'true');
    replacement.dispatchEvent(new p.w.KeyboardEvent('keydown', {key:' ', bubbles:true, cancelable:true}));
    assert.equal(clicked, 2);
    p.w.document.querySelector('.bactive-selector-fallback').click();
    assert.equal(replacement.getAttribute('tabindex'), null);
    assert.equal(first.getAttribute('role'), null);
    assert.equal(replacement.getAttribute('aria-label'), 'Slide 1');
    replacement.dispatchEvent(new p.w.KeyboardEvent('keydown', {key:'Enter', bubbles:true}));
    assert.equal(clicked, 2);
    p.close();
});
