...
try {
    ...
    $p=new WC_Product_Variable(); $p->set_name('Isolated catalogue editor regression'); $p->set_status('draft'); $p->save();
    ...
    $v=new WC_Product_Variation(); $v->set_parent_id($p->get_id()); $v->set_attributes(['pa_colour'=>'black']); $v->set_regular_price('100'); $v->set_status('publish'); $v->save();
