...
$cronNow = 1700000000;
$GLOBALS['test_options']['bactive_brevo_cron_evidence'] = [];
$assert(Config::cron_status($cronNow)['state'] === 'not_recorded' && !Config::cron_ready(), 'missing CLI evidence is not treated as fresh cron');
$GLOBALS['test_options']['bactive_brevo_cron_evidence'] = ['count' => 1, 'last' => $cronNow - 30];
$assert(Config::cron_status($cronNow)['state'] === 'insufficient_ticks', 'one CLI tick cannot establish scheduler readiness');
$GLOBALS['test_options']['bactive_brevo_cron_evidence'] = ['count' => 2, 'last' => $cronNow - Config::CRON_FRESHNESS_SECONDS - 1];
$assert(Config::cron_status($cronNow)['state'] === 'stale', 'old CLI evidence is reported stale');
$GLOBALS['test_options']['bactive_brevo_cron_evidence'] = ['count' => 2, 'last' => $cronNow + 1];
$assert(Config::cron_status($cronNow)['state'] === 'clock_skew', 'future cron evidence cannot satisfy readiness');
$GLOBALS['test_options']['bactive_brevo_cron_evidence'] = ['count' => 2, 'last' => time()];
$assert(Config::cron_status()['fresh'], 'two recent CLI ticks provide fresh cron evidence');
